...
The AppleClang and Clang (libc++) legs currently allow failure on `std::views::cartesian_product`, which libc++ hasn't implemented yet (in progress: [llvm/llvm-project#111215](https://github.com/llvm/llvm-project/pull/111215)). The MSVC legs currently allow failure on an ambiguous partial-specialization bug (`basic_compass<chequer{...}>`), [fixed upstream in VS 18.7.0](https://developercommunity.visualstudio.com/t/VS-2019-164:-Ambiguous-partial-speciali/865810) but not yet present on GitHub Actions' runner image ([actions/runner-images#14130](https://github.com/actions/runner-images/issues/14130) tracks a bump to 18.6.x, still short of the fix); there's no VS2022 leg since that's the 17.x line, which won't receive an 18.x-line fix.
