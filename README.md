...
If you've already installed tabula yourself (for example with `cmake --install`, or through your own package-manager integration), use `find_package` instead:
...
If you vendor the source directly (for example as a git submodule), use `add_subdirectory`. tabula's own tests, examples, and their Boost.Test dependency are built only when tabula is the top-level project, so consumers need not disable them:
...
The target publishes tabula's public headers and requires C++23. Individual headers can also be included directly, such as `<tabula/games.hpp>` or `<tabula/format.hpp>`.
