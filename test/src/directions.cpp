...
constexpr auto grids = std::tuple{rectangle{1, 1}, rectangle{1, 2}, rectangle{2, 1}, rectangle{2, 2}, rectangle{2, 3}, rectangle{3, 2}, rectangle{2, 4}, rectangle{4, 2}, rectangle{3, 3}, rectangle{3, 5}, rectangle{5, 3}, chequer{1, 1, 0}, chequer{1, 2, 0}, chequer{2, 1, 0}, chequer{2, 2, 0}, chequer{2, 3, 0}, chequer{3, 2, 0}, chequer{2, 4, 0}, chequer{4, 2, 0}, chequer{3, 3, 0}, chequer{3, 5, 0}, chequer{5, 3, 0}, chequer{1, 1, 1}, chequer{1, 2, 1}, chequer{2, 1, 1}, chequer{2, 2, 1}, chequer{2, 3, 1}, chequer{3, 2, 1}, chequer{2, 4, 1}, chequer{4, 2, 1}, chequer{3, 3, 1}, chequer{3, 5, 1}, chequer{5, 3, 1}};
...
BOOST_AUTO_TEST_CASE_TEMPLATE(CardinalDirectionsAreCardinal, Index, Indices)
{
        ...
        auto computed = compass::directions | std::views::filter([](auto d) { return d.is_cardinal(); });
        auto expected = std::array{compass::N, compass::E, compass::S, compass::W} | std::views::transform([](auto d) { return compass::directions[d]; });
...
BOOST_AUTO_TEST_CASE_TEMPLATE(OrdinalDirectionsAreOrdinal, Index, Indices)
{
        constexpr auto grid = std::get<Index::value>(grids);
        using compass = basic_compass<grid>;
        auto computed = compass::directions | std::views::filter([](auto d) { return d.is_ordinal(); });
        auto expected = std::array{compass::NE, compass::SE, compass::SW, compass::NW} | std::views::transform([](auto d) { return compass::directions[d]; });
        BOOST_CHECK(std::ranges::equal(computed, expected));
}
